...
export default function useRecipes() {
  const { user } = useAuth();
  const { session, updateSession, addShownRecipeId } = useSession();
  ...
  const startPipeline = useCallback(async (sessionContext) => {
    ...
    setError(null);
    try {
      console.log('[useRecipes] Calling getRecommendations for userId:', user.id);
      const result = await getRecommendations(user.id, sessionContext);
      console.log('[useRecipes] Pipeline result:', { poolSize: result.poolSize, recipeCount: result.recipes?.length });
      setRecipes(result.recipes || []);
      updateSession({ sessionPoolId: result.sessionPoolId });
      setPoolInfo({ poolSize: result.poolSize, shownCount: result.shownCount });

      (result.recipes || []).forEach((r) => addShownRecipeId(r.id));
      await trackEvent({
        userId: user.id,
  ...
  }, [user?.id, updateSession, addShownRecipeId]);
  ...
  }, [session.sessionPoolId, addShownRecipeId]);
